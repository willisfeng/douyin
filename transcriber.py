...
def log(msg):
    print(f"[{datetime.now(tz=CST).strftime('%Y-%m-%d %H:%M:%S')}] {msg}", flush=True)
...
def upload_result(filepath):
    ...
    if not filepath or not os.path.exists(filepath) or not GH_REPO or not GH_TOKEN:
        return
    release_tag = f"trans-{datetime.now(tz=CST).strftime('%Y%m%d')}"
    subprocess.run(["gh","release","create",release_tag,"--repo",GH_REPO,"--title",f"转写 {datetime.now(tz=CST).strftime('%Y-%m-%d')}","--notes","SenseVoice自动转写","--target","main"],
